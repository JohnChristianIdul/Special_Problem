...
class TimeSeriesLoader(Dataset):
    def __init__(self, df, selected_features, timestamp_col, kalman_col, sequence_length):
        ...
        self.train_data = self.train_data[self.selected_features + ['kalman_estimate', 'pm2.5']]
        self.train_data.dropna(inplace=True)

    def clean_data(self):
        # Handle missing values with time-based interpolation
        self.train_data.interpolate(method='time', inplace=True)
    ...
    def add_temporal_features(self):
        ...
        self.train_data['day_of_month'] = self.train_data.index.day
        self.train_data['week'] = self.train_data.index.isocalendar().week
    ...
    def __getitem__(self, idx):
        ...
        inputs = self.train_data.iloc[idx:end_idx].drop(columns=['pm2.5']).values.T
        label = self.train_data['pm2.5'].iloc[end_idx]
        return {
